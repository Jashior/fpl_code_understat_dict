...
class Logger {
  ...
  start() {
    ...
    const header = `--- ${this.title} | ${date} ${time} ---`;
    ...
    elements.forEach((element) => {
      ...
      if (player) {
        ...
        if (minutes > 0 && !player.Understat_ID) {
          logger.warn(
            `${fplName} (${web_name}, ${teamName}) has ${minutes} mins but no Understat_ID.`
          );
        }
      } else {
        const newRow = {
...
async function updateUnderstatIdsFromMasterCSV(logger) {
  ...
  try {
    ...
    localData.forEach((player) => {
      if (player.Code) {
        ...
        if (
          ...
          logger.warn(
            `Mismatch ID for ${player.FPL_Name}. Local: ${player.Understat_ID}, Master: ${understatId}. Overwriting.`
          );
          player.Understat_ID = understatId;
          mismatchedCount++;
        }
    ...
    if (mismatchedCount > 0) {
      logger.summary(
        `${mismatchedCount} mismatched IDs found and updated from Master CSV.`
      );
    }
    if (updatedCount > 0) {
      logger.summary(
        `${updatedCount} new Understat IDs added from Master CSV.`
      );
    }
...
async function main() {
  ...
  logger.start();
  try {
    const UPDATE_UNDERSTAT_IDS = true;
